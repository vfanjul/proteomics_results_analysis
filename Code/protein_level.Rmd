...
```{r euler, fig.height=3/0.9, fig.width=4/0.9, fig.cap=c("Euler diagram showing common dysregulated proteins in each condition.", "Euler diagram showing common upregulated proteins in each condition.", "Euler diagram showing common downregulated proteins in each condition.")}
...
if (sat_lim_95) {
  sat_lim <- round(quantile(abs(unlist(prot_data2[, sample_values, with = FALSE])), 0.95))
}
...
plot_prot_heatmap(prot_data2, sample_values, case_change, exp_design$color, "prot_lab", sat_lim)
```
...
gsea_go_long <- get_gsea_long(gsea_go_sig, prot_data[get(np_col) > 1 & !Artifact], 
                              case_params, prot_col)

gsea_kegg_sig <- get_gsea_sig(gsea_kegg, p_col = p_col)
gsea_kegg_long <- get_gsea_long(gsea_kegg_sig, prot_data[get(np_col) > 1 & !Artifact], 
                                case_params, prot_col)
...
plot_gsea_ridges(gsea_go_long, gsub("z .*", "z", case_params)[1], 
                 p_col = p_col, sat_lim = sat_lim)
```
...
for(i in case_params) {
  ...
  plot_gsea_cnet(get_gsea_group(gsea_go, prot_data[get(np_col) > 1 & !Artifact], i, 
                                p_col = p_col), sat_lim = sat_lim)
  cat("  \n")
}
```
...
plot_gsea_ridges(gsea_kegg_long, gsub("z .*", "z", case_params)[1], 
                 p_col = p_col, sat_lim = sat_lim)
```
...
for(i in case_params) {
  ...
  plot_gsea_cnet(get_gsea_group(gsea_kegg, prot_data[get(np_col) > 1 & !Artifact], i, 
                                p_col = p_col), sat_lim = sat_lim)
  cat("  \n")
}
```
...
***Note:** The complete tables of GSEA results are provided as a separate file, which contains:*

* *group: Case comparison.*
* *Accession: Accession number of the UniProtKB entry.*
* *ID: Gene set identifier.*
* *Description: Gene set description*.
* *setSize: Number of proteins in the gene set.*
* *enrichmentScore: Degree to which the gene set is over-represented.*
* *NES: Normalized enrichment score.*
* *pvalue: P value of the enrichment score, calculated using a permutation test.*
* *p.adjust: P value adjustment for multiple hypothesis testing.*
* *qvalues: Q values calculated for FDR control in P value adjustment.*
* *rank: Position in ranked list.*
* *leading_edge: Leading edge analysis with Tags (percentage of genes contributing to the enrichment score), List (where in the list the enrichment score is attained) and Signal (enrichment signal strength).*
* *core_enrichment: List of core enriched genes. *
* *Protein: Protein identifier in [UniProtKB FASTA format](https://www.uniprot.org/help/fasta-headers).*
* *variable: Which ∆ mean z-score or mean z-score was used for the GSEA.*
* *value: ∆ mean z-score or mean z-score value.*


```{r gsea_export}

fwrite(gsea_go_long, 
       paste0("Output/", project, " gsea_go_results.csv"), sep = ";")

write_excel(gsea_go_long, 
            paste0("Output/", project, " gsea_go_results.xlsx"), "gsea_go_results", 
            "NES", c("pvalue", "p.adjust"), "value", "setSize")
fwrite(gsea_kegg_long, 
       paste0("Output/", project, " gsea_kegg_results.csv"), sep = ";")

write_excel(gsea_kegg_long, 
            paste0("Output/", project, " gsea_kegg_results.xlsx"), "gsea_kegg_results", 
            "NES", c("pvalue", "p.adjust"), "value", "setSize")
```
